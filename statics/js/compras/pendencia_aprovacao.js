var API_BASE = '/api/relatorios/compras/pendencia-aprovacao';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    Promise.all([carregarInfo(), carregarHistoricoSync(), carregarAprovadores()])
    ...
    if (btnLimpar) {
        btnLimpar.addEventListener('click', function() {
            ...
            var aprovador = document.getElementById('filtro-aprovador');
            if (inicio)    inicio.value    = '';
            if (fim)       fim.value       = '';
            if (aprovador) aprovador.value = '';
    ...
    var selectAprov   = document.getElementById('filtro-aprovador');

    function atualizarFiltroAtivo() {
        var ativo = (inputInicio && inputInicio.value)
                 || (inputFim && inputFim.value)
                 || (selectAprov && selectAprov.value);
    ...
    if (selectAprov)  selectAprov.addEventListener('change', atualizarFiltroAtivo);
});
...
function definirBotaoCarregando(botao, carregando, texto) {
    ...
    botao.textContent = carregando
        ? (texto || 'Carregando...')
        : botao.getAttribute('data-default-label');
}
...
function getFiltros() {
    ...
    if (aprovador && aprovador.value) params += '&aprovador='   + encodeURIComponent(aprovador.value);
    return params;
}

function carregarAprovadores() {
    return fetch(API_BASE + '/aprovadores')
        .then(function(resp) {
            if (!verificarAuth(resp)) return;
            if (!resp.ok) throw new Error('Erro ao carregar aprovadores');
            return resp.json();
        })
        .then(function(data) {
            if (!data || !data.aprovadores) return;
            var select = document.getElementById('filtro-aprovador');
            if (!select) return;
            data.aprovadores.forEach(function(nome) {
        ...
        .catch(function() {
            // silencia — filtro de aprovador simplesmente fica vazio
        });
}
...
function baixarRelatorio() {
    ...
    mostrarMensagem('Aguarde, gerando relatório de pendências...', 'processando');
    ...
    fetch(API_BASE + '/download?formato=' + formato + filtros, { credentials: 'same-origin' })
        ...
        .then(function(blob) {
            ...
            a.href  = url;
            a.download = 'pendencia_aprovacao' + ext;
...
function atualizarDados() {
    ...
    mostrarMensagem('Sincronizando pendências de aprovação...', 'processando');
...
function mostrarMensagem(texto, tipo) {
    ...
    if (tipo === 'sucesso' || tipo === 'erro') {
        mensagemTimeout = setTimeout(function() { esconderMensagem(); }, 4200);
    }
}
