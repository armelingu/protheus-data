...
ITEM_CONTABIL        = '05.001'
TABELA               = 'pedidos_conta_05001'
SYNC_LOG             = 'pedidos_conta_05001_sync_log'
...
OUTER APPLY (
    ...
    FROM SCR010 APR
    LEFT JOIN SAK010 AK
        ON AK.AK_COD = APR.CR_APROV
        AND AK.D_E_L_E_T_ = ''
    WHERE APR.CR_NUM    = SC7.C7_NUM
...
QUERY_BASE     = _QUERY_BASE + "ORDER BY SC7.C7_EMISSAO DESC"
QUERY_NOVOS    = _QUERY_BASE + "  AND SC7.C7_EMISSAO >= ?\nORDER BY SC7.C7_EMISSAO DESC"
QUERY_COMPLETA = QUERY_BASE


# ---------------------------------------------------------------------------
# INSERT com upsert (sem nivel_aprovacao na chave)
# ---------------------------------------------------------------------------
INSERT_PEDIDO = f'''
    INSERT INTO {TABELA}
...
def _contar_mutacoes(conn, dados, data_corte):
    ...
    locais_rows = conn.execute(SELECT_PEDIDOS + f' WHERE {where}', params).fetchall()

    # KEY_IDX: FILIAL=1, PEDIDO_COMPRA=2, ITEM=3
    locais_by_key = {}
    for row in locais_rows:
        key = (_norm(row[1]), _norm(row[2]), _norm(row[3]))
...
def _upsert_no_sqlite(linhas, data_corte=None):
    ...
    try:
        ...
        try:
            ...
            print(f'[PEDIDOS_CONTA_05001] diff falhou ({exc_diff}); aplicando upsert cego.')
        ...
        if data_corte:
            ...
            total_local = conn.execute(f'SELECT COUNT(*) FROM {TABELA}').fetchone()[0]
    finally:
        conn.close()
...
def _construir_query_export(data_inicio=None, data_fim=None):
    ...
    sql += ' ORDER BY data_emissao DESC, pedido_compra, item'
    return sql, params
